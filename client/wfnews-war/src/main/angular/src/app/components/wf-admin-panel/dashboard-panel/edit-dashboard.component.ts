...
export class AdminEditDashboard implements OnInit {
  ...
  constructor(private publishedIncidentService: PublishedIncidentService, private cdr: ChangeDetectorRef, private snackbarService: MatSnackBar, private dialog: MatDialog) { }
  ...
  ngOnInit() {
    ...
    .then(results => {
      if (results?.collection) {
        ...
        this.situationReport.heavyEquipmentCount = this.previousSituationReport.heavyEquipmentCount;
  ...
  async publishChanges() {
    ...
    try {
      ...
      this.snackbarService.open('Failed to publish Situation report. Please wait a moment and try again. If the issue persist contact support: ' + JSON.stringify(err), 'OK', { duration: 100000, panelClass: 'snackbar-error' });
    }
