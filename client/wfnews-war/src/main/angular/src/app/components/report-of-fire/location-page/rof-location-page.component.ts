...
return false;
  ...
  initMap(smk: any) {
    ...
    const connector = () => {
        const photo = map.getCenter();
            const loc = [ this.location.coords.longitude, this.location.coords.latitude ] as LonLat;
  ...
  confirmLocation() {
    ...
    const direction = this.commonUtilityService.calculateBearing(this.location.coords.latitude, this.location.coords.longitude, this.fireLocation[0],this.fireLocation[1]);
    if (direction) {
  ...
  arrow( smk: any, start: LonLat, end: LonLat ) {
    ...
    this.distance = arrowLen;
        ...
        const startPt = T.along( T.lineString( [ start, headPt.geometry.coordinates ] ), startOffset ).geometry.coordinates;
    ...
    this.commonUtilityService.checkOnline().then((result) => {
      if(!result) {
        this.mapConfigService.getReportOfFireOfflineMapConfig().then((cfg) => {
          ...
          const loc = [ this.location.coords.longitude, this.location.coords.latitude];
          const dist = (this.reportOfFire.estimatedDistance && this.reportOfFire.estimatedDistance != 0) ? this.reportOfFire.estimatedDistance / 1000 : this.distanceEstimateMeter / 1000; //km
          ...
          const exp = turf.transformScale( poly, 1.10 );
