...
export class BansFullDetailsComponent implements OnInit {
  ...
  constructor(private cdr: ChangeDetectorRef, private appConfigService: AppConfigService, private agolService: AGOLService, private publishedIncidentService: PublishedIncidentService, private httpClient: HttpClient, private route: Route) { }

  async ngOnInit(): Promise<void> {
    await this.populateBansByID({ returnGeometry: true, returnCentroid: true, returnExtent: false });
    this.initMap();
  }

  async initMap() {
    // Create map and append data to the map component
    const location = [Number(this.banData.centroidLatitude), Number(this.banData.centroidLongitude)];
  ...
  async populateBansByID(options: AgolOptions = null) {
    this.banData = null;
    const response = await this.agolService.getBansAndProhibitionsById(this.id, options).toPromise();
  ...
  navToMap() {
    setTimeout(() => {
      this.route.navigate([ResourcesRoutes.ACTIVEWILDFIREMAP], { queryParams: { longitude: this.banData.centroidLongitude, latitude: this.banData.centroidLatitude, bansProhibitions: true } });
    }, 200);
  }
  ...
  navToForestUseRestrictions() {
    window.open('https://www2.gov.bc.ca/gov/content/safety/wildfire-status/prevention/fire-bans-and-restrictions/forest-use-restrictions', '_blank');
  }

  navToOpenBurning() {
    window.open('https://www2.gov.bc.ca/gov/content/safety/wildfire-status/prevention/fire-bans-and-restrictions/open-burning', '_blank');
  }
