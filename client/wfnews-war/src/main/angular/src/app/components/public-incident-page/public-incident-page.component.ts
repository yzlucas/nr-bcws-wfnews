...
export class PublicIncidentPage implements OnInit {
  ...
  ngOnInit() {
    this.router.queryParams.subscribe((params: ParamMap) => {
      const responseUrl = this.appConfigService.getConfig().externalAppConfig['bcWildfireResponsePage'].toString();
...
this.incident.incidentSizeEstimatedHa = this.incident.incidentSizeEstimatedHa.toLocaleString();
}
