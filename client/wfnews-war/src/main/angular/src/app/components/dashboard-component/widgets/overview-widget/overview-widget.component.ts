...
export class OverviewWidget implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    Promise.all([
      ...
      this.httpClient.get(url + 'HOLDING', { headers: { apikey: this.appConfigService.getConfig().application['wfnewsApiKey']} }).toPromise(),
      this.httpClient.get(url + 'UNDR_CNTRL', { headers: { apikey: this.appConfigService.getConfig().application['wfnewsApiKey']} }).toPromise()
      ...
      const oocIcon = L.icon({iconUrl: '/assets/images/svg-icons/out-of-control.svg', iconSize: [10, 10], shadowAnchor: [4, 32], shadowSize: [10, 10] });

      L.geoJSON(underControl, { pointToLayer(feature, latlng) {
 return L.marker(latlng, { icon: ucIcon });
}}).addTo(this.map);
      L.geoJSON(holding, { pointToLayer(feature, latlng) {
 return L.marker(latlng, { icon: holdIcon });
}}).addTo(this.map);
      L.geoJSON(outOfControl, { pointToLayer(feature, latlng) {
 return L.marker(latlng, { icon: oocIcon });
}}).addTo(this.map);
      L.geoJSON(firesOfNote, { pointToLayer(feature, latlng) {
